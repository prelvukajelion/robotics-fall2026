...
def render(st):
    ...
    if not lock:
        st.write('Use an AI assistant of your choice. Include your specification and this interface in your prompt:')
        st.code(f"Implement build_pattern(pattern_name: str) -> list[Segment] for '{pattern}'.\nSegment has linear_x (m/s), angular_z (rad/s), duration (s).\nReturn the ordered motion segments. The provided ROS wrapper sends the final stop.\nExplain assumptions and propose tests. Do not replace the wrapper or course checks.",language='text')
...
class MyPatternTests(unittest.TestCase):
    ...
    st.write('In a second virtual desktop terminal, run from the lab folder. This evaluates Python code and writes a report; it does not move the robot:')
    st.code('cd /workspace/week03_motion_frames_ai\npython3 scripts/evaluate_ai_pattern.py',language='bash')
    st.write('A failing test identifies work to do. Read the assertion or traceback, revise the relevant code, and rerun. '
             'The evaluator also checks the exact assigned geometry, command bounds, and the wrapper’s stop decision at completion.')
    ...
    st.write('Watch the robot and compare its path with your specification. The wrapper records checkpoint poses and checks low velocity after the final stop. '
             ...
             'complete a fresh full run and rerun the evaluator to obtain current live completion evidence.')
    result=ai_evaluation()
    if st.button('Refresh test evidence'): st.rerun()
    if result:
        st.table([{'Check':label,'Result':'Pass' if result.get(key) else 'Not verified / needs work'} for key,label in (
            ('unit_tests_passed','Student and supplied tests'),('shape_check_passed','Assigned geometry'),
            ('commands_bounded','Command limits'),('model_stop_passed','Stop decision'),('integration_passed','Live motion and stop'))])
    ...
    answer(st,'test_argument','What does each important test establish? Cite specific results.')
    answer(st,'remaining_limits','What do those tests not establish? Identify an additional test or condition you would need.')
